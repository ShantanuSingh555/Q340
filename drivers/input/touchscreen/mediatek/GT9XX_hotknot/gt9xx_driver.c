...
typedef struct
{
    u16 pid;                 //product id   //
    u16 vid;                 //version id   //
    u16 sid;			     //sensor id	//LINE <tp> <DATE2014804> <gt9xx tp> limi.zhang
} st_tpd_info;
#pragma pack()
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    //LINE <tp> <DATE2014627> <gt9xx> limi.zhang
...
#endif
    //LINE <tp> <DATE2014804> <gt9xx tp> limi.zhang
    tpd_info.sid = 0;
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {     
                ...
                {
                    ret = gtp_i2c_read(i2c_client_point, gesture_data, 6);
            		...
            		if (((gesture_data[2]==2))&&(gesture_data[3]==0)&&(gesture_data[4]==0)&&(gesture_data[5]==0))
					{
						GTP_INFO("Double click by homekey  light up the screen!");
            		...
            		{
						GTP_INFO("Double click by AA side or others key not to light up the screen!");
        				// clear 0x814B
        				doze_buf[2] = 0x00;
        				gtp_i2c_write(i2c_client_point, doze_buf, 3);
           			}
...
//BEGIN <tp> <DATE2014804> <add tp info> limi.zhang
...
int Gt9xx_tpd_get_fw_version( void )
{
    ...
    GTP_INFO("Gt9xx_tpd_get_fw_version: version_info =  %d",version_info);  
    return version_info;
}
...
void Gt9xx_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
    ...
    GTP_INFO("Gt9xx_tpd_get_fw_vendor_name: tpd_info.sid =  %d",tpd_info.sid);  
...
//END <tp> <DATE2014804> <add tp info> limi.zhang
...
static struct tpd_driver_t tpd_device_driver =
{
    ...
    .tpd_have_button = 0,
#endif
    //LINE <tp> <DATE20140804> <gt9xx tp version> limi.zhang
