...
#if defined(MTK_AUTO_DETECT_ALSPS) //guomingyi 20140826.
static struct sensor_init_info alsps_init_info;
#endif
/*----------------------------------------------------------------------------*/
...
//add for low_temperature
static int value_flag=0;
static int reset_value_flag=0;
static int orient_value_high=0;
static int orient_value_low=0;	
static void ltr553_reset_threshold_value(void)
{	 
	atomic_set(&ltr553_obj->ps_thd_val_high, orient_value_high);
	atomic_set(&ltr553_obj->ps_thd_val_low,  orient_value_low);	
	value_flag=1;
	APS_LOG(" ltr553_reset_threshold_value: threshold has been reset! \n");
}
...
//add for low_temperature
	value_flag=1;
	APS_LOG(" ltr553_enable_ps: value_flag:  %d\n",value_flag);
//end for low_temperature
		ltr553_ps_set_thres();
    ...
    {
	//add for low_temperature
	if(reset_value_flag!=0)
		ltr553_reset_threshold_value();
	//end for low_temperature
...
static DRIVER_ATTR(enable_als,   S_IWUGO | S_IRUGO, ltr553_show_enable_als, ltr553_store_enable_als);
...
//add for low_temperature
static int ltr553_set_first_threshold(struct ltr553_priv *obj,int ps)
{
	int first_value_high;
	int first_value_low;
	struct alsps_hw *hw = get_cust_alsps_hw();

	orient_value_high=atomic_read(&obj->ps_thd_val_high);
	orient_value_low=atomic_read(&obj->ps_thd_val_low);
	APS_LOG(" ltr553_set_first_threshold: ps= %d,orient_value_high=%d  ,orient_value_low=%d\n",ps,orient_value_high,orient_value_low);
	//reset the threshold
	if((ps+40)>orient_value_low)
	{
	    	//first_value_high=ps+hw->ps_threshold_high;
		//first_value_low=ps+hw->ps_threshold_low;
		first_value_high=orient_value_high;//modify for ps reset value
        	first_value_low=orient_value_low;
		if(first_value_high>0x7f0)
			{first_value_high=0x7ff;}
	    	atomic_set(&ltr553_obj->ps_thd_val_high, first_value_high);
	    	atomic_set(&ltr553_obj->ps_thd_val_low,  first_value_low);
		APS_LOG(" ltr553_set_first_threshold:reset  ps_thd_val_high= %d,ps_thd_val_low=%d \n",first_value_high,first_value_low);	
		reset_value_flag=1;
	}
	else
	{
		APS_LOG(" ltr553_set_first_threshold: That is ok, do not need reset \n");
		reset_value_flag=0;
	}
	value_flag=0;
...
static int ltr553_get_ps_value(struct ltr553_priv *obj, int ps)
{
	...
	APS_ERR("ltr553_get_ps_value###:ps=%d,Hight=%d,Low=%d\n", ps,(atomic_read(&obj->ps_thd_val_high)),(atomic_read(&obj->ps_thd_val_low)));

//add for low_temperature start
APS_LOG(" ltr553_get_ps_value:value_flag =%d \n",value_flag);
if(value_flag==1)
{
	ltr553_set_first_threshold(obj,ps);
}
//add for low_temperature end
...
#if defined(MTK_AUTO_DETECT_ALSPS) //guomingyi 20140826.
	if(err = ltr553_create_attr(&(alsps_init_info.platform_diver_addr->driver)))
#else
...
static int ltr553_i2c_remove(struct i2c_client *client)
{
    int err;
#if defined(MTK_AUTO_DETECT_ALSPS) //guomingyi 20140826.
	if(err = ltr553_delete_attr(&(alsps_init_info.platform_diver_addr->driver)))
#else
...
static int ltr553_local_remove(void)
{
    struct alsps_hw *hw = get_cust_alsps_hw();
    APS_FUN();
    ltr553_power(hw, 0);
...
#if defined(MTK_AUTO_DETECT_ALSPS)
     hwmsen_alsps_sensor_add(&alsps_init_info);
#else
