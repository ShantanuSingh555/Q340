...
#if (defined(CONFIG_PROJECT_S4710_MMX)|| defined(CONFIG_PROJECT_S4700_MMX_IN) || defined(CONFIG_PROJECT_S5301_MMX_IN) \
     || defined(CONFIG_PROJECT_S5400_MMX_IN) || defined(CONFIG_PROJECT_S4800_MMX_IN))    //weizhengliang add for mmx full charging voltage
kal_bool g_charging_full_first_flag = KAL_FALSE;
#endif
...
#if (defined(CONFIG_PROJECT_S4710_MMX)|| defined(CONFIG_PROJECT_S4700_MMX_IN) || defined(CONFIG_PROJECT_S5301_MMX_IN) \
     || defined(CONFIG_PROJECT_S5400_MMX_IN) || defined(CONFIG_PROJECT_S4800_MMX_IN))  //weizhengliang add for mmx full charging voltage
                        g_charging_full_first_flag = KAL_TRUE;
#endif
...
#if (defined(CONFIG_PROJECT_S4710_MMX)|| defined(CONFIG_PROJECT_S4700_MMX_IN) || defined(CONFIG_PROJECT_S5301_MMX_IN) \
     || defined(CONFIG_PROJECT_S5400_MMX_IN) ||defined(CONFIG_PROJECT_S4800_MMX_IN))    //weizhengliang add for mmx full charging voltage
                if(g_charging_full_first_flag == KAL_TRUE)
                    BMT_status.UI_SOC = 99;
                else
		    BMT_status.UI_SOC = BMT_status.SOC;
                battery_xlog_printk(BAT_LOG_CRTI, "[iris] UI_SOC=(%d), SOC=(%d), g_charging_full_first_flag =(%d)\n", BMT_status.UI_SOC, BMT_status.SOC, g_charging_full_first_flag);
#else
                BMT_status.UI_SOC = BMT_status.SOC;
#endif
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
		...
		#if defined(CONFIG_PROJECT_S5400) || defined(CONFIG_PROJECT_S4800)|| defined(CONFIG_PROJECT_S5222)
...
#if (defined(CONFIG_PROJECT_S4710_MMX)|| defined(CONFIG_PROJECT_S4700_MMX_IN)|| defined(CONFIG_PROJECT_S5301_MMX_IN) \
     || defined(CONFIG_PROJECT_S5400_MMX_IN) || defined(CONFIG_PROJECT_S4800_MMX_IN))    //weizhengliang add for mmx full charging voltage
        g_charging_full_first_flag = KAL_FALSE;
#endif
...
kal_uint32 maxPgFlag = 0;
kal_int32 batteryAveragePg=0;
kal_int32 batteryPgCounterIndex = 0;
kal_int32 batteryPgCounterIndex2 = 0;
#endif
void mt_battery_GetBatteryData(void)
{
...
#if defined(CONFIG_TINNO_QUICK_CHARGING)
	kal_int32 pg, v;
	static kal_int32 pg_sum;
	static kal_int32 batteryPgBuffer[BATTERY_AVERAGE_SIZE];
	static kal_uint8 batteryPgIndex = 0;
#endif
	...
	pg = v*ICharging;
	battery_xlog_printk(BAT_LOG_CRTI, "maxPgFlag start =%d, \n", maxPgFlag);
	if((ICharging>0)&(v>0)&(pg>1000))
	{
	    batteryAveragePg=mt_battery_average_method(&batteryPgBuffer[0], pg, &pg_sum, batteryPgIndex);
	    batteryPgIndex++;
...
static void mt_battery_notify_VCharger_check(void)
{
	...
	#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
	#if defined(CONFIG_PROJECT_S5400) || defined(CONFIG_PROJECT_S4800)|| defined(CONFIG_PROJECT_S5222)
	if (BMT_status.charger_vol > V_CHARGER_MAX_NOTIFY) {
	#else
	if (BMT_status.charger_vol > V_CHARGER_MAX) {
	#endif
	#else
...
void BAT_thread(void)
{
    ...
    //LINE<JIRA_ID><DATE20130123><BUG_INFO>zenghaihui
    if(g_boot_mode == FACTORY_BOOT || g_boot_mode == RECOVERY_BOOT)
    { 
        
        battery_xlog_printk(BAT_LOG_FULL, "BMT_status.charger_exist =%d, BMT_status.charger_type = %d \n", BMT_status.charger_exist, BMT_status.charger_type);
        battery_xlog_printk(BAT_LOG_FULL, "BMT_status.UI_SOC =%d, BMT_status.SOC = %d \n", BMT_status.UI_SOC, BMT_status.SOC);
        ...
        {
            battery_xlog_printk(BAT_LOG_FULL,  "[BATTERY] [LEDS]LK: leds_deinit: LEDS off \n");
            mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED, LED_OFF);
            mt65xx_leds_brightness_set(MT65XX_LED_TYPE_GREEN, LED_OFF);
            mt65xx_leds_brightness_set(MT65XX_LED_TYPE_BLUE, LED_OFF);
        }
